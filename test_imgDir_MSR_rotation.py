...
rotationAngles = [0, -10, 10, -5, 5,-15, 15] # multi1
...
for i, line in enumerate(test_list):
	...
	for rotation in rotationAngles:
		...
		image = cv2.warpAffine(imageOri, M, (image_width, image_height))
		...
		for scale in scales:
			...
			for i in xrange(top_conf.shape[0]):
				...
				score = top_conf[i]
				result = str(xmin) + ',' + str(ymin) + ',' + str(xmax) + ',' + str(ymax)  + ',' + str(rotation) + ',' + str(score) + '\r\n'
