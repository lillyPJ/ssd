...
model_def = caffe_root + 'models/VGGNet/MSRATD500/SSD_300x300/deploy.prototxt'
model_weights = caffe_root + 'models/VGGNet/MSRATD500/SSD_300x300/VGG_MSRATD500_SSD_300x300_iter_50000.caffemodel'
...
save_fig_featureMap_dir = 'fig_featureMap'
makedirs(save_fig_featureMap_dir)
...
for scale in scales:
	...
	# print feature_map
	# for ii, each in enumerate(feature_map):
	#     layer = net.blobs[each[0]].data
	#     print '{}:{}'.format(each[0], layer.shape)
	#     if (layer.ndim > 3):
	#         vis_square(layer[0, :16], imgName= '{}/{}_{}.png'.format(save_fig_featureMap_dir, ii, each[0]))
